...
#include <math.h>
#include "MAX30105.h"
#include <cstring> // 加入 memcpy
#include "esp_timer.h" // 用於獲取時間
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "esp_log.h"

// static const char* TAG = "MAX30105"; // Unused variable
...
bool MAX30105::begin(TwoWire &wirePort, uint32_t i2cSpeed, uint8_t i2caddr) {
  ...
  // Skip calling wirePort.begin() since it's already called in main application
  // Just set the clock speed
...
void MAX30105::bitMask(uint8_t reg, uint8_t mask, uint8_t thing)
{
	...
	// uint8_t originalContents = readRegister8(_i2caddr, reg); // Unused variable

	// Zero out the portions of the register we don't want to change
	// originalContents = originalContents & mask; // Unused variable
	
	// writeRegister8(_i2caddr, reg, originalContents | thing);
    // Simplified version:
    uint8_t currentContents = readRegister8(_i2caddr, reg);
    currentContents &= mask; // Apply mask to keep desired bits
    writeRegister8(_i2caddr, reg, currentContents | thing); // Write back with new bits set
}
